...
public class BounceConst {
  // Флаг режима отладки проверки столкновений (collision detection)
  public static final boolean CD_DEBUG = false;
  // Флаг режима отладки движка KJM (Nokia Game API / игровая библиотека)
  ...
  // Статус анимации: покой (шар не движется)
  public static final int MOVE_STAT = 0;
  // Состояние шара: маленький размер
  public static final int SMALL_SIZE_STATE = 0;
  // Состояние шара: большой размер
  public static final int LARGE_SIZE_STATE = 1;
  // Диаметр шара в нормальном состоянии, пиксели
  public static final int NORMAL_SIZE = 12;
  // Радиус шара в нормальном состоянии, пиксели
  public static final int HALF_NORMAL_SIZE = 6;
  // Диаметр шара в сдутом состоянии, пиксели
  public static final int POPPED_SIZE = 12;
  // Радиус шара в сдутом состоянии, пиксели
  public static final int HALF_POPPED_SIZE = 6;
  // Диаметр шара в увеличенном состоянии (после бонуса), пиксели
  public static final int ENLARGED_SIZE = 16;
  // Радиус шара в увеличенном состоянии (после бонуса), пиксели
  public static final int HALF_ENLARGED_SIZE = 8;
  // Состояние шара: нормальный (живой и активный)
  public static final int BALL_STATE_NORMAL = 0;
  // Состояние шара: погибший (жизнь потеряна)
  public static final int BALL_STATE_DEAD = 1;
  // Состояние шара: сдутый (не может нормально прыгать/двигаться)
  public static final int BALL_STATE_POPPED = 2;
  // Количество кадров анимации сдутого шара
  public static final int POPPED_FRAMES = 5;
  // Длительность действия бонуса, кадры
  public static final int BONUS_DURATION = 300;
  // Толщина тонкой платформы (tile), пиксели
  public static final int THIN_TILE_SIZE = 4;
  // Начальная сила прыжка шара, пиксели на кадр (отрицательное значение = вверх)
  public static final int JUMP_STRENGTH = -67;
  // Прирост силы прыжка при удержании кнопки, пиксели на кадр
  public static final int JUMP_STRENGTH_INC = -10;
  // Сила прыжка при активном бонусе, пиксели на кадр
  public static final int JUMP_BONUS_STRENGTH = -80;
  // Ускорение свободного падения в нормальных условиях, пиксели на кадр²
  public static final int NORMAL_GRAVITY_ACCELL = 4;
  // Ускорение падения под водой, пиксели на кадр²
  public static final int UWATER_GRAVITY_ACCELL = 6;
  // Ускорение падения для увеличенного шара, пиксели на кадр²
  public static final int LARGE_GRAVITY_ACCELL = 3;
  // Ускорение под водой для увеличенного шара, пиксели на кадр²
  public static final int LARGE_UWATER_GRAVITY_ACCELL = -2;
  // Базовое значение гравитации, используемое как опорное при расчётах
  ...
  public static final int UI_COLOUR = 545706;  // Цвет фона HUD панели
  
  public static final int BONUS_COLOUR = 16750611;  // Цвет полоски бонуса
  
  public static final int TEXT_COLOUR = 16777214;  // Цвет текста (счет, номер уровня)
  
  public static final int SCORE_TEXT_POS_X = 64;  // НE ИСПОЛЬЗУЕТСЯ в оригинальном коде
  
  public static final int SCORE_TEXT_POS_Y = 100;  // НE ИСПОЛЬЗУЕТСЯ в оригинальном коде
  
  public static final int LEVEL_TEXT_POS_X = 44;  // НE ИСПОЛЬЗУЕТСЯ в оригинальном коде
  
  public static final int LEVEL_TEXT_POS_Y = 84;  // НE ИСПОЛЬЗУЕТСЯ в оригинальном коде
  
  public static final int BONUS_SEG_SIZE = 30;  // НE ИСПОЛЬЗУЕТСЯ в оригинальном коде
  
  public static final int LEVEL_DISPLAY_TIME = 120;  // НE ИСПОЛЬЗУЕТСЯ в оригинальном коде
  
  public static final int SPLASH_NONE = -1;   // НE НУЖНО: используем enum GameState
  
  public static final int SPLASH_NOKIA = 0;   // НE НУЖНО: используем enum GameState  
  
  public static final int SPLASH_GAME = 1;    // НE НУЖНО: используем enum GameState
  ...
  // Интервал основного игрового таймера в миллисекундах (33 FPS)
