#!/usr/bin/env python3
"""
Конвертер глифов из glyphs9.txt в font9.c
"""
import re
...
def extract_glyphs_from_file(filename):
    ...
    for match in matches:
        ...
        if bits_match:
            ...
            # Фильтруем только символы с ровно 9 строками (размер font9)
            if lines and len(lines) == 9:
                ...
                # Конвертируем биты в u8 для высоты 9
                byte_data = bits_to_bytes(lines, 9)
...
def main():
    ...
    print("Extracting glyphs from glyphs9.txt...")
    glyphs = extract_glyphs_from_file('glyphs9.txt')
    ...
    # Генерируем C код
    print("Generating font9.c...")
    content = '''#include "font9.h"
...
'''
    ...
    with open('src/font9.c', 'w', encoding='utf-8') as f:
        f.write(content)

    print(f"Generated font9.c with {non_empty} glyphs (height=9)")
