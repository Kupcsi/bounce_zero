...
static texture_t* g_tileset = NULL;
static int g_tiles_per_row = 0;


// --- Dynamic sprite validation ---
static inline int get_max_sprite_index(void) {
    if (!g_tileset || g_tiles_per_row <= 0) return -1;
    // actual_height используется вместо height, так как может быть больше из-за POT требований PSP
    int tiles_per_col = g_tileset->actual_height / TILE_SIZE;
    return g_tiles_per_row * tiles_per_col;
}
...
static hoop_fg_item_t g_hoop_fg[RING_FG_QUEUE_MAX];
...
static inline png_xform_t map_tf_to_png(int tf){
    switch (tf){
        ...
        case TF_ROT_270_FLIP_X: return PNG_XFORM_ROT_270_FLIP_X;
        case TF_ROT_270_FLIP_Y: return PNG_XFORM_ROT_270_FLIP_Y;
...
texture_t* level_get_tileset(void) { return g_tileset; }
int level_get_tiles_per_row(void) { return g_tiles_per_row; }
...
static void level_load_tileset_once(void) {
    if (g_tileset) return;
    g_tileset = png_load_texture_vram(TILESET_PATH);
...
int level_load_from_file(const char* filename) {
    FILE* file = file_open_with_fallback(filename, "rb");
    if (!file) return 0;
...
static void render_exit_tile(int tile_id, float destX, float destY, int worldTileX, int worldTileY) {
    ...
    if (tile_id == 9) { // EXIT - новая логика по якорю exitPos
        ...
        if (local_x == 0 && local_y == 0) {
            ...
            const int col = t->sprite_index % g_tiles_per_row;
            const int row = t->sprite_index / g_tiles_per_row;
            
            int srcX = col * TILE_SIZE;
            int srcY = row * TILE_SIZE;
            sprite_rect_t r = png_create_sprite_rect(g_tileset, srcX, srcY, TILE_SIZE, TILE_SIZE);
            
            int animationOffset = game_get_exit_animation_offset();
            float doorX = destX;
            ...
            if (doorY < areaTop) {
                ...
                if (clipOffset < TILE_SIZE) {
                    ...
                    sprite_rect_t clipped_r = png_create_sprite_rect(g_tileset, srcX, srcY + (int)clipOffset, TILE_SIZE, (int)visibleHeight);
                    ...
                    png_draw_sprite(g_tileset, &clipped_r, doorX, areaTop, (float)TILE_SIZE, visibleHeight);
                    
                    // (1,0) - правый верхний с clipping  
                    png_draw_sprite_xform(g_tileset, &clipped_r, doorX + TILE_SIZE, areaTop, (float)TILE_SIZE, visibleHeight, PNG_XFORM_FLIP_X);
                    
                    // (0,1) и (1,1) остаются без clipping если дверь не поднялась слишком высоко
                    if (doorY + TILE_SIZE >= areaTop) {
                        png_draw_sprite_xform(g_tileset, &r, doorX, doorY + TILE_SIZE, (float)TILE_SIZE, (float)TILE_SIZE, PNG_XFORM_FLIP_Y);
                        png_draw_sprite_xform(g_tileset, &r, doorX + TILE_SIZE, doorY + TILE_SIZE, (float)TILE_SIZE, (float)TILE_SIZE, PNG_XFORM_ROT_180);
                    }
                }
            } else {
                ...
                png_draw_sprite(g_tileset, &r, doorX, doorY, (float)TILE_SIZE, (float)TILE_SIZE);
                png_draw_sprite_xform(g_tileset, &r, doorX + TILE_SIZE, doorY, (float)TILE_SIZE, (float)TILE_SIZE, PNG_XFORM_FLIP_X);
                png_draw_sprite_xform(g_tileset, &r, doorX, doorY + TILE_SIZE, (float)TILE_SIZE, (float)TILE_SIZE, PNG_XFORM_FLIP_Y);
                png_draw_sprite_xform(g_tileset, &r, doorX + TILE_SIZE, doorY + TILE_SIZE, (float)TILE_SIZE, (float)TILE_SIZE, PNG_XFORM_ROT_180);
            }
        }
...
static void render_moving_spikes_tile(int tileX, int tileY, float destX, float destY) {
    ...
    if (offsetX > -3 * TILE_SIZE && offsetX < TILE_SIZE && offsetY > -3 * TILE_SIZE && offsetY < TILE_SIZE) {
        ...
        int col = t->sprite_index % g_tiles_per_row;
        int row = t->sprite_index / g_tiles_per_row;
        ...
        for (int dy = 0; dy < 2; dy++) {
            for (int dx = 0; dx < 2; dx++) {
                ...
                if (spriteX < destX + TILE_SIZE && spriteX + TILE_SIZE > destX &&
                    ...
                    png_draw_sprite_xform(g_tileset, &r, spriteX, spriteY, (float)TILE_SIZE, (float)TILE_SIZE, xf);
                }
            }
        }
    }
...
static void render_hoop_tile(const TileMeta* t, float destX, float destY, int flags, int tileID) {
    ...
    if (tileID >= 13 && tileID <= 28 && is_sprite_valid(t->sprite_index)) {
        ...
        png_xform_t xf = map_tf_to_png(bg_transform);
        
        png_draw_sprite_xform(g_tileset, &r, destX, destY, (float)TILE_SIZE, (float)TILE_SIZE, xf);
    }
            ...
            if (g_tileset && g_tiles_per_row > 0) {
                ...
                if (tile_id == 9) {
                    ...
                    if (is_sprite_valid(sprite_idx)) {
                        ...
                        png_xform_t xf = map_tf_to_png(t->transform);
...
static int gRespawnX = 0, gRespawnY = 0;
...
void level_deactivate_old_checkpoint(void) {
    if (gRespawnX >= 0 && gRespawnX < g_level.width && gRespawnY >= 0 && gRespawnY < g_level.height) {
        uint8_t currentId = level_get_id(gRespawnX, gRespawnY);
        if (currentId == TILE_CHECKPOINT) {  // Если это активный чекпоинт (7)
            level_set_id(gRespawnX, gRespawnY, TILE_CHECKPOINT_ON);  // Меняем на неактивный (8)
        }
    }
}
...
void level_set_respawn(int tx, int ty) {
    gRespawnX = tx;
    gRespawnY = ty;
    // TODO: добавить level_get_respawn_pos() когда будет реализована система смерти/респавна
}

// --- Cleanup function for resource deallocation ---
void level_cleanup(void) {
    if (g_tileset) {
        png_free_texture(g_tileset);
        g_tileset = NULL;
        g_tiles_per_row = 0;
    }
}
