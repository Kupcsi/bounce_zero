...
void graphics_clear(u32 color) {
    // ВНИМАНИЕ: эта функция принудительно устанавливает scissor на полный экран
    // и оставляет его в таком состоянии (PSP GU не позволяет читать текущий scissor)
    sceGuScissor(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
    ...
    if ((c & 0xE0) == 0xC0) {
        ...
        *bytes_read = 2;
        ...
        switch (unicode_char) {
            ...
            case 0x0401: return 134; // Ё (после Е)
            case 0x0416: return 135; // Ж
            ...
            case 0x042F: return 160; // Я

            // Строчные а-я (позиции 161-193)
            ...
            case 0x0451: return 167; // ё (после е)
            case 0x0436: return 168; // ж
...
void graphics_draw_text_scaled(float x, float y, const char* text, u32 color, int scale) {
    ...
    while (text[i] != '\0') {
        ...
        // Рисуем символ напрямую (9 строк для font9)
        ...
        cur_x += (glyph->width + FONT9_SPACING) * scale;
        i += bytes_read;
    }
}

void graphics_draw_text(float x, float y, const char* text, u32 color) {
    graphics_draw_text_scaled(x, y, text, color, 1);
}
