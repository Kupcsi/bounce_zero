...
static bool g_exit_opening = false;      // флаг активной анимации
...
static inline void draw_centered_splash(texture_t* tex) {
    ...
    sprite_rect_t sprite = png_create_sprite_rect(tex, 0, 0, tex->width, tex->height);
    float x = (SCREEN_WIDTH - tex->width) / 2.0f;
    float y = (SCREEN_HEIGHT - tex->height) / 2.0f;
    png_draw_sprite(tex, &sprite, x, y, tex->width, tex->height);
...
void game_init(void) {
    ...
    g_game.nokia_splash_texture = png_load_texture(SPLASH_NAME_NOKIA);
    g_game.bounce_splash_texture = png_load_texture(SPLASH_NAME_BOUNCE);
...
void game_update(void) {
    switch(g_game.state) {
        ...
        case STATE_GAME: {
            ...
            if (p->ballState == BALL_STATE_DEAD) {
                ...
                if (g_game.numLives < 0) {
                    ...
                    BallSizeState currentSize = p->sizeState;  // СОХРАНЯЕМ размер
                    
                    // Респаун в стартовой точке с сохранённым размером
                    player_init(p, g_level.startPosX, g_level.startPosY, currentSize);
            ...
            if (g_exit_opening) {
                ...
                if (g_exit_animation_offset >= 24) {
                    ...
                    g_exit_opening = false;        // анимация завершена
                }
            }
        ...
        case STATE_LEVEL_COMPLETE:
            // X = продолжить (следующий уровень или меню)
...
void game_render(void) {
    ...
    switch(g_game.state) {
        ...
        case STATE_SPLASH: {
            ...
            float text_width = graphics_measure_text(press_start_text, 1);
            float text_x = (SCREEN_WIDTH - text_width) / 2.0f;
            
            graphics_draw_text(text_x, text_y, press_start_text, COLOR_TEXT_NORMAL);  // черный текст
            break;
        }
        ...
        case STATE_GAME: {
            ...
            if (s_currentCameraY == CAMERA_UNINITIALIZED) {
                ...
                int gameAreaHeight = SCREEN_HEIGHT - HUD_HEIGHT;
                s_currentCameraY = p->yPos - gameAreaHeight / 2;
            }
            ...
            float score_x = (SCREEN_WIDTH - graphics_measure_text(score_buffer, 1)) / 2.0f;
            float score_y = hudStartY + 5.0f;  // Поднял на 1 пиксель
            graphics_draw_text(score_x, score_y, score_buffer, COLOR_WHITE_ABGR);
            
            // Полоска бонуса - справа от счета для симметрии
            {
                float text_width = graphics_measure_text(score_buffer, 1);
        ...
        case STATE_LEVEL_COMPLETE:
            // Экран завершения уровня (как в Java displayLevelComplete)
            graphics_begin_plain();
            
            // Фон
            graphics_draw_rect(0.0f, 0.0f, (float)SCREEN_WIDTH, (float)SCREEN_HEIGHT, BACKGROUND_COLOUR);
            
            // Белая панель 240x128 по центру экрана (480x272)
            float panel_x = (480 - 240) / 2.0f;  // 120
            float panel_y = (272 - 128) / 2.0f;  // 72
            graphics_draw_rect(panel_x, panel_y, 240.0f, 128.0f, COLOR_WHITE_ABGR);
            
            float center_x = 480.0f / 2.0f;  // 240
            float text_y = panel_y + 20.0f;   // Отступ от верха панели
            ...
            // Кнопка Continue (убираем Back to menu)
            {
...
void game_open_exit(void) {
    if (!g_exit_opening) {  // запускаем только если еще не открывается
        g_exit_opening = true;
