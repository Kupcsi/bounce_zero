    ...
    if (player->is_playing) {
        if (player->note_time >= player->note_duration) {
            ...
            if (player->is_playing) {
                ...
                player->phase_inc     = frequency_to_phase_inc(player->frequency, player->sample_rate);
                player->phase         = 0;
                player->env_q15       = 0; // мягкий вход новой ноты
            }
        }
    }
...
int sound_init(void) {
    ...
    FILE *up_file = file_open_with_fallback(SOUND_HOOP_NAME, "rb");
    if (!up_file) {
        return 0; // Ошибка загрузки файла
    }
    ...
    FILE *pickup_file = file_open_with_fallback(SOUND_PICKUP_NAME, "rb");
    if (!pickup_file) {
        return 0; // Ошибка загрузки файла
    }
    ...
    FILE *pop_file = file_open_with_fallback(SOUND_POP_NAME, "rb");
    if (!pop_file) {
        return 0; // Ошибка загрузки файла
    }
